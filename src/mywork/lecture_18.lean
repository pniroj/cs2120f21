...
--proposition that s1 is a subset of s2
...
{a | a ∈ s ∧ p a}
--set of values that are in s and also satisfy p 
/-
...
/-
 ...
 FIX
-/
--takes in a value lambda a and returns false
def empty_set' : set α := 
  λ (a : α), false
...
a.
-/
--set of b such that everything in b is in a
...
to every value in s. 
-/
-- f is a propositon, β is a return type 
def image (f : α → β) (s : set α) : set β :=
{b | ∃ a, a ∈ s ∧ f a = b}
-- the set of b such that there exists some a in the set s and a propostion f
-- of a is equal to b
--if you're given a set of values, you can apply it set to set
